...
export default function POSPage() {
  ...
  const { products, categories, productsLoading, isOffline } = usePOSData();
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategoryId !== null) {
      result = result.filter((p: POSProduct) => p.categoryId === selectedCategoryId);
    }

    const q = deferredSearchTerm.trim().toLowerCase();
    if (q) {
      result = result.filter((p: POSProduct) => 
    ...
    return result;
  }, [products, deferredSearchTerm, selectedCategoryId]);
